...
function Page(){
 ...
 async function sync(force=false){
  if(!player.current||suppress.current||!initialized.current||!room?.id)return
 ...
 useEffect(()=>{
  if(!room?.id||!room.videoId)return
  let cancelled=false
  ...
  const make=()=>{
   ...
   player.current=new window.YT.Player(el,{videoId:room.videoId,playerVars:{playsinline:1,controls:1,rel:0,enablejsapi:1,origin:window.location.origin,widget_referrer:window.location.origin},events:{
 ...
 return <main className="min-h-screen bg-gradient-to-br from-purple-950 via-black to-pink-950 p-4 text-white"><div className="mx-auto max-w-2xl">
  ...
  {!room.otherPresent&&<button onClick={()=>void resendInvite()} className="mt-3 w-full rounded-2xl bg-white/10 py-3">📨 Invite partner again</button>}
  ...
  <div className="mt-4 rounded-2xl border border-white/10 bg-black/20 p-3">
   ...
   {(selected==="liked"||selected==="watchLater"||selected==="search"||playlistVideos.length>0)&&<div className="mt-3 space-y-2">
    {selected==="search"&&<div className="space-y-2"><div className="flex gap-2"><input value={search} onChange={e=>setSearch(e.target.value)} onKeyDown={e=>e.key==="Enter"&&void searchVideos()} placeholder="Search YouTube" className="min-w-0 flex-1 rounded-xl bg-white/10 px-3 py-3 outline-none"/><button onClick={()=>void searchVideos()} disabled={searching} className="rounded-xl bg-white/10 px-4">{searching?"…":"Search"}</button></div><div className="flex gap-2"><input value={paste} onChange={e=>setPaste(e.target.value)} placeholder="Paste YouTube link" className="min-w-0 flex-1 rounded-xl bg-white/10 px-3 py-3 outline-none"/><button onClick={()=>{const id=extractId(paste);if(id)choose(id)}} className="rounded-xl bg-white/10 px-4">Load</button></div>{searchMessage&&<p className="rounded-xl bg-white/5 p-3 text-sm text-white/60">{searchMessage}</p>}</div>}
    {selected!=="search"&&displayed.length===0&&<p className="py-4 text-center text-sm text-white/40">No videos here yet.</p>}
    {(selected==="search"?searchResults:displayed).map(v=><button key={v.id} onClick={()=>chooseVideo(v)} className="flex w-full gap-3 rounded-xl bg-white/5 p-2 text-left disabled:opacity-50">{v.thumbnail?<img src={v.thumbnail} alt="" className="h-16 w-28 rounded-lg object-cover"/>:<div className="h-16 w-28 rounded-lg bg-white/10"/>}<span className="min-w-0"><span className="block truncate text-sm font-medium">{v.title||v.id}</span><span className="block truncate text-xs text-white/40">{v.channel||"YouTube"}</span></span></button>)}
