...
export async function GET(){
  ...
  const events=await prisma.calendarEvent.findMany({where:{homeId:c.m.homeId},orderBy:{startAt:"asc"}})
...
export async function POST(req:Request){
  ...
  if(b.action==="delete"){
    await prisma.calendarEvent.deleteMany({where:{id:String(b.id),homeId:c.m.homeId}})
    return NextResponse.json({ok:true})
  }
  if(b.action==="update"){
    const id=String(b.id)
    const existing=await prisma.calendarEvent.findFirst({where:{id,homeId:c.m.homeId}})
