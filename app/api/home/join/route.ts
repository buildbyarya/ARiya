...
export async function POST(request: Request) {
  ...
  const existingHome = await prisma.homeMember.findUnique({
    where: {
      userId: user.id,
    },
  })
  ...
  const pendingHome = await prisma.pendingHome.findUnique({
    where: {
      inviteCode: code,
    },
  })
