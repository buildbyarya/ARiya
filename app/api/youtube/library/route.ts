...
async function getContext(){
 ...
 const user=await prisma.user.findUnique({where:{email:email}});if(!user)return null
...
export async function POST(request:Request){
 const context=await getContext();if(!context)return NextResponse.json({error:"Unauthorized"},{status:401})
 const body=await request.json();const videoId=String(body?.videoId||"").trim();const type=normalizeType(body?.type)
 ...
 const created=await prisma.libraryVideo.create({data:{homeId:context.home.id,videoId,type,title:null,thumbnail:null,channel:null}})
