...
export async function POST(req:Request){
  try{
    ...
    if(b.action==="create-invite"){
      ...
      let setting
      try{setting=await prisma.watchSetting.upsert({where:{userId:c.user.id},create:{userId:c.user.id},update:{}})}catch(e){console.error("Watch Together setting error",e);return NextResponse.json({error:"Could not access Watch Together settings."},{status:500})}
